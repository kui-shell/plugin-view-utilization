 ...
 *
 */
function parse(data: string, raw = false): Tables.Table {
  const header = {
    name: 'Container',
    attributes: [
      { value: 'Pod' },
  ...
  const body = data.split(/\n/).map(line => {
    const [name, namespace, container, ip, cpuRequests, memoryRequests, cpuLimits, memoryLimits ] = line.split(/\t/)
...
export default async function getNodeData(options: Commands.ParsedOptions, raw = false): Promise<Tables.Table> {
  ...
  return parse(await REPL.rexec<string>(cmd), raw)
}
